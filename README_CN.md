...
本人 fork 自 [arXiv-Pulse](https://github.com/kYangLi/ArXiv-Pulse)（github.com/kYangLi/ArXiv-Pulse）的个人自用版本。仅供自己使用，不提供维护与支持，不接受 issues/PR。

## 本地定制说明

- 新增期刊来源：IEEE TGRS、Science（Crossref 元数据 + Semantic Scholar 摘要）
- 更新与显示均支持按来源（arXiv / TGRS / Science）过滤
- 界面更名为 "ATS Pulse"
- 近期论文默认时间范围：1 天
...
**arXiv Pulse** 是一个 Python 包，用于自动爬取、总结和追踪 arXiv 上的最新论文。它支持所有 arXiv 类别，提供现代化的 Web 界面，带来专业的文献管理体验。
...
- **UI 组件增强**: 重新设计按钮、开关、选择器、对话框，优化阴影和过渡效果
- **论文序号**: 论文卡片上显示序号，便于引用
- **回到顶部按钮**: 滚动感知的浮动按钮，快速返回页面顶部
- **浮动按钮提示**: 悬停显示所有浮动按钮的功能说明
- **近期论文 AI 搜索**: 在近期论文中使用自然语言搜索
- **同步页面改进**: 优化间距，帮助图标带提示
- **SQLite WAL 模式**: 支持并发读写，性能更好
- **问题修复**: 表单提交、分页显示、搜索时序号保留
...
arXiv Pulse 支持**所有 arXiv 类别**。只需在设置页面选择你感兴趣的领域。预置选项包括：

| 类别 | 示例领域 |
|------|----------|
| 物理 | 凝聚态物理、量子物理、高能物理、核物理、天体物理 |
| 计算 | DFT、第一性原理、分子动力学、力场、计算物理 |
| AI/ML | 机器学习、人工智能、计算机视觉、自然语言处理 |
| 化学 | 量子化学、化学物理 |
| 数学 | 数学物理、数值分析、统计学 |
| 其他 | 定量生物学、电气工程、经济学 |

你也可以添加自定义搜索查询来追踪任何 arXiv 主题。
